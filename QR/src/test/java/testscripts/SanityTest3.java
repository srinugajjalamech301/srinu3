...
public class SanityTest3 {
	...
	//	Robot srin=new Robot(); 
		...
		try { Thread.sleep(2000); System.out.println("In Try Catch"); if
		  (driver.getCurrentUrl().contains("site/login")) {
		 System.out.println("In If block"); //
		 Thread.sleep(1500);
		 //https://test.lubesqr.bpcl.in/central/site/login
		 //https://dev.lubesqr.bpcl.in/central/site/login
		 
		 driver.navigate().to("http://10.10.10.192/BPCL/central/site/dashboard");
		  System.out.println("Done Refresh"); }
		  }
		  catch (Exception e) { System.out.println("Contact Adminstrator not showing");
		 }
		...
		/*
		 * srin.keyPress(KeyEvent.VK_CONTROL); srin.keyPress(KeyEvent.VK_SHIFT);
		 * 
		 * srin.keyPress(KeyEvent.VK_M); srin.keyRelease(KeyEvent.VK_M);
		 * srin.keyRelease(KeyEvent.VK_CONTROL); srin.keyRelease(KeyEvent.VK_SHIFT);
		 */
		...
		for (WebElement Li : links) {
			if (Li.getAttribute("href").contains("/central")) {
				...
				try {
					...
					try {
						...
						if (!rav) {
							...
							System.out.println(title + "-->" + totalTime + " Milliseconds or -->" + (totalTime / 1000)
									+ "  Seconds");
